...
def main() -> int:
    ...
    try:
        ...
        if not chosen_text:
            ...
            # 置換後に重複判定を再実行
            jac_final, ham_final, nearest_final = most_similar_info(chosen_text, index)
            logger.info(
                "置換後の類似度: Jaccard=%.3f, Hamming距離=%d (しきい値: Jaccard<%.2f, Hamming>%d)",
                jac_final, ham_final, JACCARD_TH, HAMMING_TH
            )
            if jac_final >= JACCARD_TH or ham_final <= HAMMING_TH:
                logger.warning(
                    "⚠ 置換後も類似度が高いままです。重複投稿のリスクがあります。"
                )
